  ...
  describe('Validation conditional checks', () => {
      ...
      constructor(childInfo: ChildInfo, name: string, persons: readonly Person[]) {
        super(validator =>
          validator.object(childInfo).then(({ fatherName, motherName }) => {
            validator.if(!fatherName, !motherName).fail('PARENTS_ARE_AVAILABLE');
            validator
              .if(!!fatherName)
              .earn('FATHER_NAME_EXISTS')
              .check('FATHER_NAME_IS_VALID', () => checkNameValidity(fatherName!))
              .with(persons.find(p => p.name === fatherName))
              .check('FATHER_IS_AVAILABLE', parent => !!parent)
              .check('FATHER_IS_A_PARENT', parent => parent.type === 'PARENT')
              .check('FATHER_IS_MY_PARENT', parent => (parent.info as ParentInfo).childrenNames.includes(name));
            validator
              .if(!!motherName)
              .earn('MOTHER_NAME_EXISTS')
              .check('MOTHER_NAME_IS_VALID', () => checkNameValidity(motherName!))
              .with(persons.find(p => p.name === motherName))
              .check('MOTHER_IS_AVAILABLE', parent => !!parent)
              .check('MOTHER_IS_A_PARENT', parent => parent.type === 'PARENT')
              .check('MOTHER_IS_MY_PARENT', parent => (parent.info as ParentInfo).childrenNames.includes(name));
          })
        );
      }
    }
    ...
    > {
      constructor(studentInfo: StudentInfo, name: string, persons: readonly Person[], schools: readonly string[]) {
        super(validator => {
          validator.put(validator.$.asChild, new ChildInfoValidation(studentInfo, name, persons));
        ...
        super(validator =>
          validator.object(parentInfo).then(({ childrenNames }) => {
            validator.array(childrenNames).each((childName, index) => {
              validator
                .put(validator.$.childrenNames[index].exists, !!childName)
                .put(validator.$.childrenNames[index].isValid, typeof childName === 'string' && checkNameValidity(childName));
            });
          })
        );
      }
          ...
          validator.object(person).then(({ name, age, type, info }) => {
            ...
            validator.when('AGE_IS_VALID', 'TYPE_IS_VALID').then(() => {
              validator.if(type === 'CHILD').then(() => {
                validator.put(validator.$.childInfo, new ChildInfoValidation(info as ChildInfo, name, persons));
                validator.check('IF_CHILD_AGE_IS_UNDER_7', age < 7);
              });
              validator.if(type === 'STUDENT').then(() => {
                validator.put(validator.$.studentInfo, new StudentInfoValidator(info as StudentInfo, name, persons, schools));
              ...
              validator.if(type === 'PARENT').then(() => {
                validator.put(validator.$.parentInfo, new ParentInfoValidation(info as ParentInfo));
