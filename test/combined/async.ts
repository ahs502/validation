...
describe('Validation', () => {
  describe('Validation basic async checks', () => {
        ...
        super(validator =>
          validator.object(user).then(({ id, username }) => {
            ...
            validator
              ...
              .with(checkUsernameAsync(id, username))
              .then(exists => validator.check('USERNAME_IS_VALID', exists))
              .when('USERNAME_IS_VALID')
              .with(checkUserAccessAsync(id))
              .then(access => validator.check('USER_HAS_ACCESS', access));
          })
        );
      }
    }
    ...
    test.each`
      ...
      ${{ id: 1005, username: 'psghdjks' }} | ${true}     | ${undefined} | ${'User access not found.'}
      ${{ id: 1006, username: 'abcdefgh' }} | ${true}     | ${undefined} | ${'User not found.'}
