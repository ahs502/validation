  ...
  describe('Validation for nested data structures', () => {
    ...
    class PointValidation extends Validation<'X_IS_VALID' | 'X_IS_NOT_NEGATIVE' | 'Y_IS_VALID' | 'Y_IS_NOT_NEGATIVE'> {
      constructor(point: Point) {
        super(validator =>
          validator.object(point).then(({ x, y }) => {
    ...
    > {
      constructor(path: Path) {
        super((validator, validation) =>
          validator.object(path).then(({ length, points }) => {
            validator
            ...
            validator.array(points).each((point, index) => validator.$('points', index).set(new PointValidation(point)));
            validator
